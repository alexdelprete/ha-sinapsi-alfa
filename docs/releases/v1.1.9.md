...
1. **Early abort on cascade** - If 3+ protocol errors occur in a single update cycle, the cycle aborts early instead of
continuing to retry all 5 batches

**Before:** A cascade of protocol errors could take 2-3 minutes to recover as each of the 5 batches would burn through
...
Transaction ID mismatch after timeouts is a fundamental limitation of Modbus TCP
that affects all libraries (ModbusLink, pymodbus, libmodbus). The root fix requires upstream library changes:
...
[v1.1.8...v1.1.9](https://github.com/alexdelprete/ha-sinapsi-alfa/compare/v1.1.8...v1.1.9)
