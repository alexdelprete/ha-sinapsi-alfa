...
1. **Sensor Availability**: Disconnect network/device, verify sensors show "unavailable"
1. **Config Flow**: Try invalid inputs (wrong ports, invalid IPs, extreme scan intervals)
1. **Error Logging**: Check logs have consistent, clear format with context
1. **Network Recovery**: Test recovery from connection failures and timeouts
1. **Options Flow**: Change scan interval and other settings, verify they work
...
1. Replace the integration files
...
[ABB PowerOne v4.1.5](https://github.com/alexdelprete/ha-abb-powerone-pvi-sunspec/releases/tag/v4.1.5)
...
1. Include log excerpts showing the new standardized log format
