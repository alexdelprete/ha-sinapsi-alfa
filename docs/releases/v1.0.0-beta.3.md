...
- **Fixed IPv4/IPv6 dual-stack connection timeout** - `check_port()` now forces IPv4 (`socket.AF_INET`) to avoid
  timeouts when the device only supports IPv4 but DNS returns IPv6 addresses first

## Technical Details

**Root Cause**: `asyncio.open_connection()` performs dual-stack DNS resolution.
When a hostname resolves to both IPv4 and IPv6 addresses, Python tries IPv6 first.
If the device only supports IPv4, the connection waits for the full timeout (3 seconds) before falling back.
