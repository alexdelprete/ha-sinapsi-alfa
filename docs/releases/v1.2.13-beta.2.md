...
- **Sync timeout counter poisoned by idle polls** - The beta.1 sync mechanism's timeout counter
  ...
  firmware finally updated production (only one sensor fresh), the counter could already be at 1,
  causing the timeout to fire immediately and calculate `prodotta - immessa` with desynchronized
  values. This produced oscillation with a ~50% hit rate per firmware cycle.
...
1. Monitor `sensor.alfa_energy_self_consumed` during solar production hours (10:00-14:00 CET)
2. The history graph should show smooth monotonic increase - NO sawtooth dips
3. Utility meters (`sensor.daily_energy_autoconsumption`) should not show sudden jumps

## Technical Details

- **Files changed:** `api.py` (sync logic), `test_api.py` (updated + regression test)
- **Test coverage:** 98% (899 statements, 212 branches)
