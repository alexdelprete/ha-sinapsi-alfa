...
- **Fixed Transaction ID mismatch errors** - Replaced parallel `asyncio.gather()` with sequential batch reads to avoid
  Modbus TCP protocol violations

## Technical Details

**Root Cause**: `asyncio.gather()` sent all 5 batch reads in parallel over the same TCP connection.
Modbus TCP uses transaction IDs to match requests/responses, but concurrent requests caused ID mismatches:

```text
事务ID不匹配 | Transaction ID mismatch: 期望 | Expected 5, 收到 | Received 1
```

**Fix**: Batch reads are now executed sequentially. This still provides 75% fewer requests than individual register
reads (5 batches vs 20 individual reads).
