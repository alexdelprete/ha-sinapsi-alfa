...
This official stable release brings **ha-sinapsi-alfa** up to date with the latest Home Assistant 2025.10 requirements,
Python 3.13 support, and includes comprehensive code quality improvements and critical bug fixes
that were tested during the beta period.
...
1. Click "Update" in HACS
1. Restart Home Assistant
1. Verify all sensors are working correctly

#### Option 2: Manual Update

1. Stop Home Assistant (or disable the integration)
1. Download the v0.5.0 release zip file
1. Extract and replace the integration files in `custom_components/sinapsi_alfa/`
1. Restart Home Assistant (or re-enable the integration)
1. Verify all sensors are working correctly
...
1. **0eeed4c** - Bump softprops/action-gh-release from 2.4.0 to 2.4.1 (#161)
1. **700ea1f** - Bump stefanzweifel/git-auto-commit-action from 6.0.1 to 7.0.0 (#160)
1. **0b00fad** - Improve code formatting and readability
1. **ae1e5e8** - Update Ruff target version to Python 3.13
1. **1ef7ff0** - Release v0.5.0-beta.1: Align with ABB PowerOne v4.1.5 improvements
1. **b3234c6** - Bump ruff from 0.13.3 to 0.14.0 (#159)
1. **86a749b** - Bump softprops/action-gh-release from 2.3.4 to 2.4.0 (#158)
1. **c13fb17** - Bump ruff from 0.13.2 to 0.13.3 (#157)
1. **d4bb79e** - Bump softprops/action-gh-release from 2.3.3 to 2.3.4 (#156)
1. **907e4c9** - Bump ruff from 0.13.1 to 0.13.2 (#154)
...
1. **Calculated sensors**: Check that calculated values (potenza_consumata, etc.) are accurate
1. **Integration reload**: Test reload from Settings → Devices & Services
1. **Home Assistant restart**: Verify integration loads correctly after restart
...
This release incorporates improvements from the
[ABB Power-One PVI SunSpec](https://github.com/alexdelprete/ha-abb-powerone-pvi-sunspec) integration v4.1.5,
which underwent 6 beta iterations to ensure stability.
...
1. **Verify requirements**: Ensure HA 2025.10.0+ with Python 3.13+
1. **Report issues**: [GitHub Issues](https://github.com/alexdelprete/ha-sinapsi-alfa/issues)
1. **Community forum**: [Home Assistant Community](https://community.home-assistant.io/t/custom-integration-alfa-by-sinapsi-data-integration/705294)
